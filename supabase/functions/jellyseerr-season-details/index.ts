...
serve(async (req) => {
  ...
  try {
    ...
    if (!tvId || seasonNumber === undefined) {
      return new Response(JSON.stringify({ error: 'TV ID og sesong nummer er påkrevd' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    let response;
    try {
      response = await fetch(seasonDetailsUrl, { method: 'GET', headers: { 'X-Api-Key': jellyseerrApiKey, 'Content-Type': 'application/json' } });
    } catch (fetchError) {
