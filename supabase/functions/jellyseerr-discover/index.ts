...
serve(async (req) => {
  ...
  try {
    ...
    if (!type || !['movie', 'tv'].includes(type)) {
      return new Response(JSON.stringify({ error: 'Type må være "movie" eller "tv"' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    const params = new URLSearchParams({ page: String(page), language: 'no' });
    ...
    let response;
    try {
      response = await fetch(discoverUrl, { method: 'GET', headers: { 'X-Api-Key': jellyseerrApiKey, 'Content-Type': 'application/json' } });
    } catch (fetchError) {
