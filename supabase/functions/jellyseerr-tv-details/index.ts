...
serve(async (req) => {
  ...
  try {
    ...
    if (!tvId) {
      return new Response(JSON.stringify({ error: 'TV ID er påkrevd' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    let response;
    try {
      response = await fetch(tvDetailsUrl, { method: 'GET', headers: { 'X-Api-Key': jellyseerrApiKey, 'Content-Type': 'application/json' } });
    } catch (fetchError) {
